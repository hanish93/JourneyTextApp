...
def load_models(device):
    ...
    proc,mod = get_caption_models(device)
    scene_mod,scene_cls = get_scene_model(device)
    print("[Models] Done.")
    return {
        "yolo": yolo, "ocr": ocr,
        "cap_proc": proc, "cap_mod": mod,
...
def process_frames(src, M):
    ...
    prev_gray=None
    imgs = (
        (cv2.imread(f) for f in sorted(glob(os.path.join(src,"*.jpg"))))
        if os.path.isdir(src) else extract_frames(src)
    )
...
def run_pipeline(src):
    ...
    for row in summarise_journey(motions,lm,cap,scn,ocr,signals):
        print(f"[{row['step']:03d}] {row['event']:<20} | "
